...
export const route = (instance: typeof server) => {
    instance
        .get("/:id_pelaksanaan_pelatihan", { // id pelaksanaan pelatihan
            ...
            const res = await db.select().from(tablePeserta).where(and(eq(tablePeserta.id_pelaksanaan_pelatihan, id_pelaksanaan_pelatihan), eq(tablePeserta.id, id_peserta))).execute();

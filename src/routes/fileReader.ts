...
export const route = (instance: typeof server) => {
    instance
        .get("/e-materi/:filename", {
            ...
            try {
                const getSertifikat = await db.select(
                    {
                ...
                .leftJoin(pelaksanaanPelatihan, eq(sertifikat.id_pelaksanaan_pelatihan, pelaksanaanPelatihan.id))
                .leftJoin(alias(users, "instruktur"), eq(pelaksanaanPelatihan.id_instruktur, alias(users, "instruktur").id))
                ...
                const page = pdfDoc.addPage([template.bitmap.width, template.bitmap.height]);
                ...
                const heveticeFont = await pdfDoc.embedFont(StandardFonts.TimesRomanBold);

                const addTextToTemplate = (text: string, x: number, y: number, font: PDFFont, size: number, color = rgb(0, 0, 0)) => {
                ...
                const ExpiredDate = masaBerlaku.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' });
                ...
                addTextToTemplate(getSertifikat[0].nama ? getSertifikat[0].nama : '', 0, 250, heveticeFont, 64);
                addTextToTemplate('has completed the training of', 0, 100, fontStyle, 36);
                addTextToTemplate(getSertifikat[0].nama_pelatihan ? getSertifikat[0].nama_pelatihan : '', 0, 0, heveticeFont, 64);
                addTextToTemplate('expired on', 0, -180, fontStyle, 36);
                addTextToTemplate(getSertifikat[0].masa_berlaku ? ExpiredDate : '', 0, -250, fontStyle, 36);
                addTextToTemplate(`Bandung, ${formattedDate}`, 360, -500, fontStyle, 36);
                addTextToTemplate('Head of Human Capital Division', 350, -550, heveticeFont, 36);
                addTextToTemplate('Muhammad Hafidz', 360, -870, heveticeFont, 36);
                addTextToTemplate('Instruktur', -460, -550, heveticeFont, 36);
                addTextToTemplate(getSertifikat[0].nama_instruktur ? getSertifikat[0].nama_instruktur : '', -450, -870, heveticeFont, 36);
              ...
              reply
                ...
                .header('Content-Disposition', `inline; filename=certificate_${getSertifikat[0].sertifikasi}.pdf`)

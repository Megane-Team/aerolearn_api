...
import {
  permintaanTraining,
  permintaanTrainingSchema,
} from "@/models/draft_permintaan_training.ts";
    ...
    .put(
      ...
      async (req) => {
        ...
        const permintaan = await db
          ...
          .where(eq(permintaanTraining.id, Number(id)))
          .execute();
        ...
        if (status == "tolak") {
          const training = await db
            ...
            .where(
              eq(pelaksanaanPelatihan.id, permintaan[0].id_pelaksanaanPelatihan)
            )
            .execute();
          if (!training) {
            return {
